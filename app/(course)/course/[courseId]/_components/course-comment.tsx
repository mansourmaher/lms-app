...
import { CourseReview, User } from "@prisma/client";
...
interface CommentListProps {
  comments: Awaited<ReturnType<typeof getCourseComments>>;
}
    ...
    <>
      <div>
        {comments.map((comment) => (
          <ol key={comment.id} className="relative border-s border-gray-200 dark:border-gray-700">
              ...
              <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-700 dark:border-gray-600">
                ...
                <div>
                    <div className="flex items-center justify-between mt-3">
                        ...
                        <div className="flex items-center space-x-2">
                        <button
                        onClick={() => ReactOnComment(true,comment.id).then(()=>window.location.reload())}
                         className="text-xs font-semibold text-gray-500 hover:underline dark:text-gray-300">
                            {comment.likes} Like
                        </button>
                        <button 
                        onClick={() => ReactOnComment(false,comment.id).then(()=>window.location.reload())}
                        className="text-xs font-semibold text-gray-500 hover:underline dark:text-gray-300">
